...
object LocalLauncher extends App {
  ...
  val local: ActorRef = SystemManager.getInstance().createActor(IntActor.props("Local"), "local")
  //NULL POINTER LA RIGA SOTTO CIOCCA IL GETACTOR
  //val remoteActor: ActorRef = SystemManager.getInstance().getLocalActor("akka.tcp://RemoteSystem@192.168.1.12:2727/user/remote")
  val remoteActor: ActorSelection = SystemManager.getInstance().getRemoteActor("RemoteSystem","192.168.1.12","2727", "/user/remote")
